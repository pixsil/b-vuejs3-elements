...
export default {
    ...
    methods: {
        updateArray() {
            let new_model_arr = toRaw(this.modelValue);
            if (new_model_arr.includes(this.value)) {
                var index = new_model_arr.indexOf(this.value);
                if (index !== -1) {
                    new_model_arr.splice(index, 1);
                }
            } else {
                new_model_arr.push(this.value);
            }

            this.$emit('update:modelValue', new_model_arr);
        },
    },
...
.custom-control-input:checked ~ .custom-control-label:before {
    background-color: #078e1d;
    border-color: #078e1d;
    color: #fff
}

.custom-control-input:focus ~ .custom-control-label:before {
    box-shadow: 0 0 0 .2rem rgba(7, 142, 29, .25)
}
...
.custom-range:focus::-webkit-slider-thumb {
    box-shadow: 0 0 0 1px #f8fafc, 0 0 0 .2rem rgba(7, 142, 29, .25)
}
